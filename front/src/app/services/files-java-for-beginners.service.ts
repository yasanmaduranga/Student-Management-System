...
export class FilesJavaForBeginnersService {
  ...
  getJavaFileListByUsernameandCourse(course, username) {
    return this.http.get('http://localhost:3000/java_files/getByUsernameandCourse/' + course + '/' + username);
  }
  ...
  deleteFileAll(course){
    return this.http.delete('http://localhost:3000/java_files/deleteAll/' + course);
  }
