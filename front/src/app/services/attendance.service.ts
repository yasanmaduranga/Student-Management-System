...
export class AttendanceService {
  ...
  deleteAttendanceForUnregister(username){
    return new Promise((resolve, reject) => {
     this.http.delete('http://localhost:3000/attendance_management/unregister/' + username)
       //.map(res => res.json())
       .subscribe(res => {
         resolve(res);
       }, (err) => {
 ...
 deleteAttendanceForUnenrol(username, course){
  return new Promise((resolve, reject) => {
   this.http.delete('http://localhost:3000/attendance_management/unenrol/' + username + '/' + course)
     //.map(res => res.json())
     .subscribe(res => {
       resolve(res);
     }, (err) => {
